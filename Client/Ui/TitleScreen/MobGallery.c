                                ...
                                struct rr_game *game)
{
    ...
    if (data->secondary_animation > 0.999)
        return;
    rr_ui_set_tooltip_metadata(game->mob_tooltips[data->id][data->rarity],
...
struct rr_ui_element *rr_ui_mob_container_init()
{
    struct rr_ui_element *this = rr_ui_2d_container_init(
        rr_rarity_id_max, rr_mob_id_pectinodon + 1, 15, 15);
    for (uint8_t id = 0; id <= rr_mob_id_pectinodon; ++id)
